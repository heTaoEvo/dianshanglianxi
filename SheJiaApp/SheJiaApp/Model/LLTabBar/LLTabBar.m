...
- (void)config {
	...
	topLine.image = [UIImage imageNamed:@"footer_bg"];
...
- (void)itemSelected:(LLTabBarItem *)sender {
    ...
    if (sender.tabBarItemType != LLTabBarItemRise) {
        {
            if(self.delegate)//点击按钮掉用方法
            {
                if ([self.delegate respondsToSelector:@selector(tabBarDidSelectedNormalItem:)]) {
                    [self.delegate tabBarDidSelectedNormalItem:sender.titleLabel.text];
                }
            }
...
- (void)setTabBarItems:(NSArray *)tabBarItems {
	...
	for (id item in tabBarItems) {
		if ([item isKindOfClass:[LLTabBarItem class]]) {
            ...
            LLTabBarItem *button = item;
            NSLog(@"%f",CGRectGetHeight(button.bounds));
